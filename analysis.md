...
## Round 2026-06-05 — カードデバッグのルール準拠修正（スキル墓地/PP/発火導線）

### 修正内容

- `js/dev/cardDebug.js`
  - 手札カードの操作ボタンを `cardKind` に応じて制限
    - `attacker/support` のみ `ATK` 配置可
    - `skill/support` のみ `SKILL` 使用可
  - `SKILL` 使用時の前提を追加
    - 場に `attacker/support` が無い場合は使用不可ログを表示
  - スキル使用後の挙動をゲーム寄せに修正
    - `resolveCardOnPlay(..., "skill")` 後に `resolveCardOnLeave(...)` を実行し、最終的に墓地へ移動
  - 以前のデバッグ用PPバイパス（`ppCostHandled=1`）を撤去
    - デバッグでもPP消費不足時は通常どおり不発になる
  - サポートをスキルとして使用する経路では、デバッグ内のみ `cost=1` 扱いでPP消費を再現

### 意図

- 「カードデバッグで効果フローが呼ばれていないように見える」主因だった
  - コスト無視の挙動
  - 種別に関係なく操作できる導線
  - スキル使用後に場へ残る差分
  を、実ゲーム導線へ近づけて検証結果の信頼性を上げる。

## Round 2026-06-05 — カードデバッグ再調整（閉じる操作/発火安定化/PP/アタッカーのスキル運用）
...
- `js/dev/cardDebug.js`
  - 閉じる操作を追加
    - オーバーレイ外クリックで閉じる
    - `Esc` キーで閉じる
  - 効果フロー不発の主因だった `instanceId` 欠落を修正
    - デバッグ生成カードに `dataset.instanceId` を付与
    - `PlayerActionResolver` の重複実行ガードに引っかからず、カードごとに効果判定されるよう修正
  - PPバイパスを使わず、通常のコスト処理を通す構成を維持
  - スキル使用導線を拡張
    - 手札アタッカーに `SKILL` 操作を追加
    - 場に他アタッカーがいる時のみ、アタッカーをスキルとして使用可
    - スキル使用後は `onLeave` 処理を通して墓地へ移動

- `js/game/auto/playerActionResolver.js`
  - `resolveCardOnPlay` のカード種別判定を拡張
    - `zoneType === "skill"` かつ「他アタッカーが場にいる」場合、アタッカーを `skill` 扱いで実行
    - 条件未達なら警告して使用をキャンセル
  - デバッグ用オーバーライド（`debugCardKind` / `debugCostOverride`）を受けられるようにし、デバッグと本編の実行経路を統一

### 期待結果

- カードデバッグで「ほぼ発動しない」状態が改善
- カード使用時に PP が減少
- スキルカードは使用後に墓地へ遷移
- 追加仕様どおり、他アタッカーがいる場合に手札アタッカーをスキル運用可能

## Round 2026-06-05 — 効果判定ログの可視化（NONE系は非表示）

### 変更

- `js/game/auto/playerActionResolver.js`
  - `logEffectJudgement(...)` を追加し、実際に判定対象になった効果ごとにログ出力。
  - 出力内容:
    - `判定=発動`
    - `判定=条件不成立`
    - `判定=対象不成立`
    - `判定=不発`（上記以外の不成立）
  - `NONE` / `NONE_DSL` など「効果定義がない」系の記録を抑止。
...
## Round 2026-06-09 — CardEditor UI再編（右端ドロワー化 / カード見た目編集 / 右クリック追加導線）

### 反映内容

- 旧ブロック表示をInspectorから除外。
  - `Inspector` はノード編集に集中し、`legacyToggleBtn` 導線をUIから削除。

- 右端の縦タブ + 展開ドロワーを実装。
  - 常時表示: `Event Engine / Runtime Inspector / Replay Debugger / Log Viewer / Output Preview` タイトルのみ。
  - クリック時: 最上層モーダルとして展開し、対象ビューを表示。
  - 既存表示IDを維持（`eventEngineViewer`, `runtimeInspectorView`, `replayView`, `ideLogViewer`, `outputPreview`）。

- ノードエディタ右クリック改善。
  - 右クリックメニューに `Node Library` を常時表示（空白右クリック・ノード右クリック双方）。
  - ライブラリ項目クリックで `ノード設定モーダル` を開いて追加。

- 左カラムをカード一覧/ノード一覧からカード見た目プレビューへ変更。
  - `Card Preview` パネルにカードモック表示を追加。
  - その上部に `カードメニュー` ボタンを追加。
  - メニュー内に `[保存 / カード選択 / 追加 / 削除]` を格納。

- カード見た目のインタラクティブ編集を実装。
  - クリック対象: `種別 / カード外枠 / カード画像 / 属性 / カード名 / 攻撃力 / 効果テキスト`。
  - ホバー時に半透明アウトライン + ラベル表示。
  - クリック時に `cardFieldModal` を開き、対象項目をフォーム編集。
...
- 追加対応（2026-06-20）: デッキ構築画面の右上デッキ詳細パネル（単押し選択時）ボタンUIを調整。
  - 文言変更: 「編集」→「デッキを編集」、「公開」→「デッキを公開」。
  - 配置変更（2x2）: 左上=スリーブ変更 / 右上=デッキを編集 / 左下=削除 / 右下=デッキを公開。
  - 公開ボタンの配色を既存ボタンスタイル維持のまま水色系グラデーションへ変更。
- 追加対応（2026-06-20）: 公開処理に確認モーダルを追加。
...
- 追加対応（2026-06-20）: 開発者モードのカードエディタにタグ設定UIを追加（プリセット選択式）。
  - `js/dev/cardEditorIDE.js`
    ...
    - `タグ設定` モーダルを追加し、プリセットタグ一覧をトグル選択（未選択 `[タグ]` / 選択済み `]タグ[` 表示）できるように実装。
    - プリセットは固定候補 + 既存カードのタグ集合を統合して表示し、既存タグも編集可能。
    - タグ更新時は `card.tags` に即時反映し、カード一覧/出力プレビュー/情報フォームにも同期。
    - カード情報フォームのタグ入力は直入力から「タグを選択」ボタン + readOnly表示へ変更。
  - `dev.html`
...
## Round 2026-06-22 — デッキ確認表示とデッキ構築カードUI調整

### 要望

- ゲーム画面の「デッキ確認」は、進行中のデッキ順（ランダム/使用で減る状態）ではなく、
  - 同一カードを集約して枚数表示
  - ID順で表示
  - ゲーム進行に関係なく常に全カード表示
- デッキ構築画面のカード名テキストボックスが右寄りで枚数表示と重なる。
- カード表示上、アタッカー攻撃力とサポートの「-」が見えづらい。

### 対応

- `js/ui/deckViewer.js`
  - 参照元を `state[owner].deck` から `matchSetup.deckCode` / `localStorage.deckCode` のデコード結果へ変更。
  - これにより、ゲーム中の消費/順序変化に影響されず、常に開始デッキ全体を表示。
  - 表示時に `id -> count` で集約し、ID順ソートで描画。
  - 同一IDが複数ある場合に `×n` バッジを表示。
- `css/style.css`
  - `.cvName` の左右位置と内側余白を調整し、右上の枚数バッジ重なりを回避しつつ左へ寄せる。
  - `.cvAttack` と `.cvAttackNone > span` を黒系に変更し可読性を改善。
