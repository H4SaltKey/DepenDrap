(function() {
  ...
  function buildCardEditorView() {
    ...
    const EFFECT_ACTION_OPTIONS = [
      ...
      { value: "duplicate_to_hand", label: "duplicate_to_hand" },
      { value: "reveal_card", label: "reveal_card" },
      { value: "fetch_card", label: "fetch_card" },
      { value: "play_to_field", label: "play_to_field" },
      { value: "trigger_attack_effect", label: "trigger_attack_effect" },
    ...
    const ACTION_LABEL_MAP = {
      ...
      duplicate_to_hand: "複製して手札へ",
      reveal_card: "カード公開",
      fetch_card: "カード移動",
      play_to_field: "場に出す",
      trigger_attack_effect: "攻撃時効果を再実行",
      add_effect: "効果を付与",
      remove_effect: "効果を削除",
      remove_status: "状態削除"
    };
    ...
    const CONDITION_LEFT_OPTIONS = [
      ...
      { value: "event.didDirectAttack", label: "直接攻撃したか" },
      { value: "self.hp", label: "自分のHP" },
      { value: "self.pp", label: "自分のPP" },
      { value: "opponent.hp", label: "相手のHP" },
    ...
    function renderNodeConfigForm() {
      ...
      if (d.type === "trigger") {
        ...
        const help = {
          ...
          set_pp_min: "例: 1",
          add_shield: "例: 1",
          add_atk: "例: 1 atkMode=increase atkTarget=this_card",
          move_to_grave: "例: （空）",
          return_to_hand: "例: （空）",
          return_to_deck: "例: （空）",
          duplicate_to_hand: "例: （空）",
          reveal_card: "例: （空）",
          fetch_card: "例: 1 toZone=hand",
          play_to_field: "例: 1 toZone=attacker",
          trigger_attack_effect: "例: （空）",
          add_effect: "例: Shield turns=1 allowDuplicate=false",
          remove_effect: "例: Shield",
          remove_status: "例: Shield"
        };
        typeSpecific = `
          <select data-k="action">${effectOptionsHtml}</select>
          <input data-k="args" value="${htmlEscape(argsText)}" placeholder="${help[d.data?.action] || "引数を自由入力（スペース区切り）"}">
        `;
