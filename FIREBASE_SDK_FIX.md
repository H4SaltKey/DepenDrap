...
これらの URL は ES6 モジュール形式のコードを返すため、ブラウザで `import` / `export` 構文が使用されていると認識され、エラーが発生していました。
...
| 特徴 | 説明 |
|------|------|
| グローバルオブジェクト | `window.firebase` として利用可能 |
| API 互換性 | 従来の Firebase SDK と同じ API |
| モジュール形式 | グローバルスクリプト（UMD） |
| ブラウザ互換性 | すべてのブラウザで動作 |
