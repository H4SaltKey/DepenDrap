...
- 各タイミングに `bundleCondition` を設定可能
- 条件を満たしたときのみ、そのタイミング配下の効果を順番に実行

## 7.1.2 退場時の直接攻撃条件

- `onLeave` とその配下効果の条件で `directAttack` を指定可能
  - `any`（問わない）
  - `did`（直接攻撃した）
  - `not`（直接攻撃していない）
...
- `これが場にある間` / `このターン中` と同階層で以下を指定可能:
  - `アタッカー場のカードのT効果によって`（T: 登場時/攻撃時/直接攻撃時/退場時/即効/継続）
  - `スキルカードの効果によって`
  - `この一連(同じタイミング内)の効果中`
- `N番目が発動したなら` を指定可能（複数選択）
  - 判定モード: `any(どれか)` / `all(すべて)`
  - 同一タイミング内の効果番号（1始まり）
  - 効果は番号の昇順で実行
  - 並べ替え時は番号に紐づく位置が更新される
...
- `owner`:
  - `自身 / 現在のターゲット / アタッカー場のカード / 使用したスキルカード / このカード`
- `mode`:
  - `current_eq`（現在値がN）
  - `current_gte`（現在値がN以上）
  - `current_lte`（現在値がN以下）
...
- スコープ:
  - `このターン中` が `true` のとき `turn`
  - `このターン中` が `false` のとき自動で `game`
  - 条件UIでの `このターン中 / ゲーム中` 選択は廃止
...
- カード対象効果:
  ...
  - `手札のカード`
- 注意:
  - `現在のターゲット` 系でターゲットがモンスターの場合、カード対象として成立しない効果は無効扱い
